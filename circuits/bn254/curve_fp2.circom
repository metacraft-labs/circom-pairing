...
template PointOnLineFp2Bn254(n, k, p) {
...
template PointOnCurveFp2Bn254(n, k, a, b0, b1, p){
...
// - a is length 2 array with entries in [0, 2^n) representing Fp2 element
// - b = [b0, b1] is length 2 x k array with entries in [0, 2^n) representing Fp2 element
template EllipticCurveFunctionBn254(n, k, a, b0, b1, p){
    signal input in[2][k];
...
// a is complex
template PointOnTangentFp2Bn254(n, k, a, p){
...
template EllipticCurveAddUnequalFp2Bn254(n, k, p) {
    ...
    component y_constraint = PointOnLineFp2Bn254(n, k, p);
...
// * x_1 != x_3
template EllipticCurveDoubleFp2Bn254(n, k, a, b0, b1, p) {
    signal input in[2][2][k];
    signal output out[2][2][k];
    ...
    component point_on_tangent = PointOnTangentFp2Bn254(n, k, a, p);
    for(var j=0; j<2; j++)for(var i=0; i<k; i++){
    ...
    component point_on_curve = PointOnCurveFp2Bn254(n, k, a, b0, b1, p);
    for(var j=0; j<2; j++)for(var i=0; i<k; i++) {
...
template EllipticCurveAddFp2Bn254(n, k, a2, b20, b21, p){
    signal input a[2][2][k];
    ...
    component add = EllipticCurveAddUnequalFp2Bn254(n, k, p);
    component doub = EllipticCurveDoubleFp2Bn254(n, k, a2, b20, b21, p);
...
template EllipticCurveScalarMultiplyFp2Bn254(n, k, b0, b1, x, p){
    signal input in[2][2][k];
    ...
    for(var i=BitLength - 1; i>=0; i--){
        if( i == BitLength - 1 ){
            ...
            if(Bits[i] == 0){
                ...
                Padd[curid] = EllipticCurveAddFp2Bn254(n, k, [0,0], b0, b1, p);
...
template EllipticCurveScalarMultiplyUnequalFp2Bn254(n, k, b0, b1, x, p){
    signal input in[2][2][k];
    signal output out[2][2][k];
