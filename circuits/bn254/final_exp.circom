...
template Fp12CyclotomicCompressBn254(n, k) {
...
template Fp12CyclotomicDecompressBn254(n, k, p) {
...
// input is [g2, g3, g4, g5] = C(g) in compressed format of Fp12CyclotomicCompressBn254
...
template SignedFp12CyclotomicSquareNoCarryBn254(n, k) {
    signal input in[4][2][k];
...
template Fp12CyclotomicSquareBn254(n, k, p) {
    ...
    component sq = SignedFp12CyclotomicSquareNoCarryBn254(n, k);
...
// assume 0 < e < 2^254
template Fp12CyclotomicExpBn254(n, k, e, p) {
    assert( e > 0 );
    ...
    // Compress in[]
    component Cin = Fp12CyclotomicCompressBn254(n, k);
    ...
    for(var i=0; i<BITLENGTH; i++){
        ...
        if( i > 0 ){ // pow2[0] is never defined since there is no squaring involved
            pow2[i] = Fp12CyclotomicSquareBn254(n, k, p);
            if( i == 1 ){
        ...
        if( ((e >> i) & 1) == 1 ){
            ...
            if( i > 0 ){
                Dpow2[curid] = Fp12CyclotomicDecompressBn254(n, k, p);
                for(var id=0; id<4; id++)for(var eps=0; eps<2; eps++)for(var j=0; j<k; j++)
...
template BNFinalExpHardPart(n, k, p){
    ...
    // m^x
    component mx = Fp12ExpBn254(n, k, x, p);
    for(var id=0; id<6; id++)for(var eps=0; eps<2; eps++)for(var j=0; j<k; j++)
        mx.in[id][eps][j] <== in[id][eps][j];

    // m^{x^2} = (m^x)^x
    component mx2 = Fp12ExpBn254(n, k, x, p);
    for(var id=0; id<6; id++)for(var eps=0; eps<2; eps++)for(var j=0; j<k; j++)
        mx2.in[id][eps][j] <== mx.out[id][eps][j];

    // m^{x^3} = (m^{x^2})^x
    component mx3 = Fp12ExpBn254(n, k, x, p);
    for(var id=0; id<6; id++)for(var eps=0; eps<2; eps++)for(var j=0; j<k; j++)
        mx3.in[id][eps][j] <== mx2.out[id][eps][j];


    // m^p
    component mp = Fp12FrobeniusMapBn254(n, k, 1);
    // m^{p^2}
    component mp2 = Fp12FrobeniusMapBn254(n, k, 2);
    // m^{p^3}
    component mp3 = Fp12FrobeniusMapBn254(n, k, 3);
    // (m^x)^p
    component mxp = Fp12FrobeniusMapBn254(n, k, 1);
    // (m^{x^2})^p
    component mx2p = Fp12FrobeniusMapBn254(n, k, 1);
    // (m^{x^3})^p
    component mx3p = Fp12FrobeniusMapBn254(n, k, 1);
    // (m^{x^2})^{p^2}
    component mx2p2 = Fp12FrobeniusMapBn254(n, k, 2);
...
// out = in^{ (q^6 - 1)*(q^2 + 1) }
template FinalExpEasyPartBn254(n, k, p){
    signal input in[6][2][k];
    signal output out[6][2][k];
    ...
    // in^{(q^6-1)*q^2} = f3^{q^2}
    component f4 = Fp12FrobeniusMapBn254(n, k, 2);
    for(var id=0; id<6; id++)for(var eps=0; eps<2; eps++)for(var j=0; j<k; j++)
...
// out = in^{(q^12-1)/r} = BNFinalExpHardPart( FinalExpEasyPartBn254(in) )
template FinalExponentiateBn254(n, k, p){
    signal input in[6][2][k];
    signal output out[6][2][k];

    component f1 = FinalExpEasyPartBn254(n, k, p);
    for(var id=0; id<6; id++)for(var eps=0; eps<2; eps++)for(var j=0; j<k; j++)
        f1.in[id][eps][j] <== in[id][eps][j];
