...
// Assumes: p = 3 (mod 4)
template Fp12FrobeniusMapBn254(n, k, power){
    signal input in[6][2][k];
    signal output out[6][2][k];

    var p[50] = get_bn254_prime(n, k);
    var FP12_FROBENIUS_COEFFICIENTS[12][6][2][20] = get_Fp12_frobeniusBn254(n, k);
    var pow = power % 12;
...
template SignedFp12ScalarMultiplyNoCarryBn254(n, k, m_out){
    signal input a[k];
    signal input b[6][2][k];
...
// m_out is the expected max number of bits in the output registers
template SignedFp12ScalarMultiplyNoCarryUnequalBn254(n, ka, kb, m_out){
    signal input a[ka];
...
// out is a*b in Fp12 as 6 x 2 x (2k-1) array
// m_out is the expected max number of bits in the output registers
template SignedFp12Fp2MultiplyNoCarryBn254(n, k, m_out){
    signal input a[2][k];
    signal input b[6][2][k];
    signal output out[6][2][2*k-1];
...
// m_out is the expected max number of bits in the output registers
template SignedFp12Fp2MultiplyNoCarryUnequalBn254(n, ka, kb, m_out){
    signal input a[2][ka];
    signal input b[6][2][kb];
    signal output out[6][2][ka+kb-1];
...
// m_out is the expected max number of bits in the output registers
template SignedFp12MultiplyNoCarryUnequalBn254(n, ka, kb, m_out){
    var l = 6;
    var XI0 = 9;
...
template SignedFp12MultiplyNoCarryBn254(n, k, m_out){
    var l = 6;
    signal input a[l][2][k];
    signal input b[l][2][k];
    signal output out[l][2][2*k-1];

    component mult = SignedFp12MultiplyNoCarryUnequalBn254(n, k, k, m_out);
...
// m_out is the expected max number of bits in the output registers
template Fp12CompressBn254(n, k, m, p, m_out){
    var l = 6;
...
// m_out is the expected max number of bits in the output registers
template SignedFp12MultiplyNoCarryCompressBn254(n, k, p, m_in, m_out) {
    var l = 6;
    signal input a[l][2][k];
    signal input b[l][2][k];
    signal output out[l][2][k];
    ...
    component nocarry = SignedFp12MultiplyNoCarryBn254(n, k, 2*m_in + LOGK1);
    for (var i = 0; i < l; i ++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++){
...
template SignedFp12CarryModPBn254(n, k, overflow, p) {
    var l = 6;
...
template Fp12MultiplyBn254(n, k, p) {
    ...
    component no_carry = SignedFp12MultiplyNoCarryCompressBn254(n, k, p, n, 3*n + LOGK2);
    ...
    component carry_mod;
    carry_mod = SignedFp12CarryModPBn254(n, k, 3*n + LOGK2, p);
    for (var i = 0; i < l; i++)for(var j = 0; j < 2; j++)
        for (var idx = 0; idx < k; idx++)
...
template Fp12MultiplyThree(n, k, p) {
    ...
    component ab = SignedFp12MultiplyNoCarryBn254(n, k, 2*n + LOGK1);
    for(var i=0; i<l; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++){
        ab.a[i][j][idx] <== a[i][j][idx];
        ab.b[i][j][idx] <== b[i][j][idx];
    }
    component abc = SignedFp12MultiplyNoCarryUnequalBn254(n, 2*k-1, k, 3*n + 2*LOGK1);
    for(var i=0; i<l; i++)for(var j=0; j<2; j++){
        for(var idx=0; idx<2*k-1; idx++)
    ...
    component carry;
    carry = SignedFp12CarryModPBn254(n, k, 4*n + LOGK3, p);
    for(var i=0; i<l; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)
...
template Fp12SquareBn254(n, k, p) {
    signal input in[6][2][k];
    signal output out[6][2][k];
...
// not actually a relevant circuit - this only exists to test find_Fp6_inverseBn254
template Fp6InvertBn254(n, k, p) {
    signal input a0[2][k];
    signal input a1[2][k];
    signal input a2[2][k];
    var out[6][2][50] = find_Fp6_inverseBn254(n, k, p, a0, a1, a2);
...
// Call find_Fp12_inverseBn254 to compute inverse
// Then check out * in = 1, out is an array of shorts
template Fp12InvertBn254(n, k, p){
    signal input in[6][2][k];
    signal output out[6][2][k];

    var inverse[6][2][50] = find_Fp12_inverseBn254(n, k, p, in); // 6 x 2 x 50, only 6 x 2 x k relevant
    for (var i = 0; i < 6; i ++) {
        for (var j = 0; j < 2; j ++) {
            for (var m = 0; m < k; m ++) {
...
template Fp12ExpBn254(n, k, e, p) {
    ...
    for(var b=BITLENGTH-1; b>=0; b--){
        ...
        if( b == BITLENGTH - 1 ){ // pow2[0] is never defined since there is no squaring involved
            ...
            // square
            square[b] = SignedFp12MultiplyNoCarryBn254(n, k, 2*n + LOGK1);
            for(var i=0; i<6; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++){
            ...
            if( ((e >> b) & 1) == 0 ){
                ...
                carry[b] = SignedFp12CarryModPBn254(n, k, 3*n + LOGK2, p);
                for(var i=0; i<6; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)
                    carry[b].in[i][j][idx] <== compress[b].out[i][j][idx];
            }else{
                // multiply square with `in`
                mult[curid] = SignedFp12MultiplyNoCarryUnequalBn254(n, 2*k-1, k, 3*n + 2*LOGK1);
                for(var i=0; i<6; i++)for(var j=0; j<2; j++){
                    for(var idx=0; idx<2*k-1; idx++)
                ...
                carry[b] = SignedFp12CarryModPBn254(n, k, 4*n + LOGK3, p);
                for(var i=0; i<6; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)
                    carry[b].in[i][j][idx] <== compress[b].out[i][j][idx];
