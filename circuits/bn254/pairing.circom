...
template SignedLineFunctionUnequalNoCarryFp2Bn254(n, k, m_out){
...
// m_out is the expected max number of bits in the output registers
template SignedLineFunctionEqualNoCarryFp2Bn254(n, k, m_out){
...
//  - line_{Psi(P0), Psi(P1)}(Q) where Psi(x,y) = (w^2 x, w^3 y)
//  - equals w^3 (y_1 - y_2) X + w^2 (x_2 - x_1) Y + w^5 (x_1 y_2 - x_2 y_1)
template LineFunctionUnequalFp2Bn254(n, k, q) {
    signal input P[2][2][2][k];
    signal input Q[2][k];
    ...
    component nocarry = SignedLineFunctionUnequalNoCarryFp2Bn254(n, k, 2 * n + LOGK1);
    ...
    component carry = SignedFp12CarryModPBn254(n, k, 3 * n + LOGK2, q);
    for (var i = 0; i < 6; i++) {
        for (var j = 0; j < 2; j++) {
            for (var idx = 0; idx < k; idx++) {
...
//  - line_{Psi(P), Psi(P)}(Q) where Psi(x,y) = (w^2 x, w^3 y)
//  - equals (3x^3 - 2y^2)(XI0 + u) + w^4 (-3 x^2 X) + w^3 (2 y Y)
template LineFunctionEqualFp2Bn254(n, k, p) {
    signal input P[2][2][k];
    signal input Q[2][k];
    ...
    component nocarry = SignedLineFunctionEqualNoCarryFp2Bn254(n, k, 3*n + LOGK2);
    for (var i = 0; i < 2; i++) {
        for (var j = 0; j < 2; j ++) {
            for (var idx = 0; idx < k; idx++) {
    ...
    component carry = SignedFp12CarryModPBn254(n, k, 4 * n + LOGK3, p);
    for (var i = 0; i < 6; i++) {
        for (var j = 0; j < 2; j++) {
            for (var idx = 0; idx < k; idx++) {
                carry.in[i][j][idx] <== reduce[i][j].out[idx];
...
//  P0, P1, Q are as in inputs of SignedLineFunctionUnequalNoCarryFp2Bn254
// Assume:
...
//  out is 6 x 2 x k
template Fp12MultiplyWithLineUnequalFp2Bn254(n, k, kg, overflowg, q){
    ...
    component line = SignedLineFunctionUnequalNoCarryFp2Bn254(n, k, 2*n + LOGK1); // 6 x 2 x 2k - 1 registers in [0, 12k 2^{2n})
    ...
    component mult = SignedFp12MultiplyNoCarryUnequalBn254(n, kg, 2*k - 1, overflowg + 2*n + LOGK2); // 6 x 2 x (2k + kg - 2) registers abs val < 12k * min(kg, 2k - 1) * 6 * (2+XI0)* 2^{overflowg + 2n}
    ...
    component carry = SignedFp12CarryModPBn254(n, k, overflowg + 3*n + LOGK3, q);
...
//  P, Q are as in inputs of SignedLineFunctionEqualNoCarryFp2Bn254
// Assume:
...
template Fp12MultiplyWithLineEqualFp2(n, k, p){
    ...
    component line = SignedLineFunctionEqualNoCarryFp2Bn254(n, k, 3*n + LOGK2); // 6 x 2 x 3k - 2 registers abs val < (12k^2 * (XI0 + 1) + 1) 2^{3n}
    for(var j=0; j<2; j++)for(var l=0; l<2; l++)for(var idx=0; idx<k; idx++)
        line.P[j][l][idx] <== P[j][l][idx];
    for(var l=0; l<2; l++)for(var idx=0; idx<k; idx++)
        line.Q[l][idx] <== Q[l][idx];

    component mult = SignedFp12MultiplyNoCarryUnequalBn254(n, k, 3*k - 2, 4*n + LOGK3); // 6 x 2 x (4k - 3) registers abs val < 12k^2 * (XI0 + 1) * 6 * (2+XI0) * 2^{4n}
    for(var i=0; i<6; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)
        mult.a[i][j][idx] <== g[i][j][idx];
    ...
    component carry = SignedFp12CarryModPBn254(n, k, 5*n + LOGK4, p);
    for(var i=0; i<6; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)
        carry.in[i][j][idx] <== reduce.out[i][j][idx];
...
template MillerLoopFp2Bn254(n, k, b0, b1, loopCount, p){
    signal input P[2][2][k];
    signal input Q[2][k];
    ...
    for(var i=BitLength - 1; i>=0; i--){
        if( i == BitLength - 1 ){
            ...
            square[i] = SignedFp12MultiplyNoCarryBn254(n, k, 2*n + 4 + LOGK); // 6 x 2 x 2k-1 registers in [0, 6 * k * (2+XI0) * 2^{2n} )
            for(var l=0; l<6; l++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++){
            ...
            nocarry[i] = SignedFp12MultiplyNoCarryUnequalBn254(n, 2*k-1, k, 3*n + LOGK2); // 6 x 2 x 3k-2 registers < (6 * (2+XI0))^2 * k^2 * 2^{3n} )
            ...
            fdouble[i] = SignedFp12CarryModPBn254(n, k, 4*n + LOGK3, p);
            for(var l=0; l<6; l++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)
                fdouble[i].in[l][j][idx] <== compress[i].out[l][j][idx];

            Pdouble[i] = EllipticCurveDoubleFp2Bn254(n, k, [0,0], b0, b1, p);
            for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)for(var l=0; l<2; l++)
            ...
            if(Bits[i] == 0){
                ...
                Padd[curid] = EllipticCurveAddUnequalFp2Bn254(n, k, p);
                for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)for(var l=0; l<2; l++){
    ...
    var coeff[12][6][2][20] = get_Fp12_frobeniusBn254(n, k);
    ...
    component line1 = LineFunctionUnequalFp2Bn254(n, k, p);
    ...
    component R_add = EllipticCurveAddUnequalFp2Bn254(n, k, p);
    for(var i=0; i<2; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++){
    ...
    component line2 = LineFunctionUnequalFp2Bn254(n, k, p);
    for(var i=0; i<2; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++){
...
template MillerLoopThreeFp2(n, k, b0, b1, loopCount, p){
    ...
    for(var i=BitLength - 1; i>=0; i--){
        if( i == BitLength - 1 ){
            ...
            for(var idP=0; idP<numPairing; idP++){
                ...
                Pdouble[i][idP] = EllipticCurveDoubleFp2Bn254(n, k, [0,0], b0, b1, p);
                for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)for(var l=0; l<2; l++)
            ...
            if(Bits[i] == 0){
                ...
                for(var idP=0; idP<numPairing; idP++){
                    ...
                    Padd[curid][idP] = EllipticCurveAddUnequalFp2Bn254(n, k, p);
                    for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)for(var l=0; l<2; l++){
    ...
    for(var idP=0; idP<numPairing; idP++){
        ...
        // l_{[loopCount] P', Frob_p(P')}(Q)
        line1[idP] = LineFunctionUnequalFp2Bn254(n, k, p);
        for(var i=0; i<2; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++){
        ...
        // l_{[loopCount] P' + Frob_p(P'), -Frob_p^2(P')}(Q)
        R_add[idP] = EllipticCurveAddUnequalFp2Bn254(n, k, p);
        for(var i=0; i<2; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++){
        ...
        line2[idP] = LineFunctionUnequalFp2Bn254(n, k, p);
        for(var i=0; i<2; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++){
...
template OptimalAtePairingBn254(n, k, p){
    signal input P[2][2][k];
    signal input Q[2][k];
    signal output out[6][2][k];
