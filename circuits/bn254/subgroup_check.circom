...
template EndomorphismPsiBn254(n, k, p){
...
template SubgroupCheckG1Bn254(n, k){
    signal input in[2][k];
...
template SubgroupCheckG2Bn254(n, k){
    signal input in[2][2][k];
    ...
    component is_on_curve = PointOnCurveFp2Bn254(n, k, [0,0], b[0], b[1], p);

    for(var i=0; i<2; i++)for(var j=0; j<2; j++)for(var idx=0; idx<k; idx++)
        is_on_curve.in[i][j][idx] <== in[i][j][idx];

    component psiP = EndomorphismPsiBn254(n, k, p);
    var lambda = 6 * x * x;
    component lambdaP = EllipticCurveScalarMultiplyUnequalFp2Bn254(n, k, b[0], b[1], lambda, p);
